    ...
    <div class="panel panel-default hidden-xs">   
        ...
        <div class="panel-body">
            <div class="list-group">
                ...
                <a href="{{ route('users.favorites',$user->id)  }}" class="list-group-item "><span class="badge badge-{{ $user->favorite_count > 0 ? 'hint' : 'fade' }}">{{ $user->favorite_count  }}</span><span class="glyphicon glyphicon-folder-open"></span>我收藏的话题</a>
                @endif
                ...
                @endif
                <a href="{{ route('users.followings',$user->id)  }}" class="list-group-item "><span class="badge badge-{{ $user->following_count > 0 ? 'hint' : 'fade' }}">{{ $user->following_count }}</span><span class="glyphicon glyphicon-eye-open"></span>{{Auth::id()==$user->id?"我关注的用户":"Ta 关注的用户"}}</a>
                <a href="{{ route('users.followers',$user->id)  }}" class="list-group-item "><span class="badge badge-{{ $user->follower_count > 0 ? 'hint' : 'fade' }}">{{ $user->follower_count }}</span><span class="glyphicon glyphicon-heart"></span>{{Auth::id()==$user->id?"关注我的用户":"关注 Ta 的用户"}}</a>
