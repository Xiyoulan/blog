<div class="col-lg-9 col-md-8 col-sm-7 app-main">
    <ul class="nav nav-tabs user-nav" id="user-tab">
        <li role="presentation" class="{{ active_class(if_route('users.show')&&!if_query('tab','edit')&&!if_query('tab','comment'))}}"><a href="#article" data-toggle="tab">{{Auth::id()==$user->id?"我的话题":"Ta 的话题"}}</a></li>
        <li role="presentation" class="{{ active_class(if_route('users.show')&&if_query('tab','comment')) }}"><a href="#comment" data-toggle="tab">评论/回复</a></li>
        @can('update',$user)
    ...
    <div class="tab-content">
        <div role="tabpanel" class="tab-pane {{ active_class(if_route('users.show')&&!if_query('tab','edit')&&!if_query('tab','comment'))}}" id="article">
            @include('users._article')
        </div>
        <div role="tabpanel" class="tab-pane {{ active_class(if_route('users.show')&&if_query('tab','comment')) }}" id="comment">
            @include('users._comment')
        </div>
        @can('update',$user)
